...
def get_user_scoring(model, user_num, test_batch_size, args, local_rank):
    model.eval()
    ...
    with torch.no_grad():
        for input_ids in user_dataloader:
            ...
            user_scoring.extend(user_feature)
    return torch.stack(tensors=user_scoring, dim=0).to(torch.device("cpu")).detach()


def get_item_scoring(model, item_content, test_batch_size, args, use_modal, local_rank):
    model.eval()
    ...
    with torch.no_grad():
        for input_ids in item_dataloader:
            ...
            item_scoring.extend(item_feature)
    return torch.stack(tensors=item_scoring, dim=0).detach()


def eval_model(model, user_history, eval_pairs, user_scoring, item_scoring, test_batch_size,
               args, item_num, Log_file, v_or_t, local_rank):
    eval_dataset = BuildEvalDataset(eval_pairs=eval_pairs, user_content=user_scoring, item_num=item_num)
    ...
    with torch.no_grad():
        ...
        for data in eval_dl:
            ...
            scores = torch.matmul(user_embs, item_scoring.t()).squeeze(dim=-1).detach()
