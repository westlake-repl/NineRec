...
def get_itemLMDB_embeddings(model, item_num, item_id_to_keys, test_batch_size, args, local_rank):
    model.eval()
    item_dataset = Build_Lmdb_Eval_Dataset(args=args, data=np.arange(item_num + 1), item_id_to_keys=item_id_to_keys,
...
def eval_model(model, user_history, eval_seq, item_embeddings, test_batch_size, args, item_num, Log_file, v_or_t, local_rank):
    ...
    with torch.no_grad():
        ...
        for data in eval_dl:
            ...
            elif 'nextitnet' in args.model_tower:
                prec_emb = model.module.user_encoder(input_embs)[:, -1].detach()
            elif 'gru4rec' in args.model_tower:
                prec_emb = model.module.user_encoder(input_embs)[:, -1].detach()
