...
class Build_Lmdb_Dataset(Dataset):
    def __init__(self, args, u2seq, item_num, max_seq_len, db_path, item_id_to_keys, resize):
        self.args = args
        self.u2seq = u2seq
        self.item_num = item_num
        self.max_seq_len = max_seq_len + 1
    ...
    def __getitem__(self, user_id):
        ...
        with env.begin() as txn:
            for i in range(tokens_Len):
                ...
                image_trans = self.transform(Image.fromarray(IMAGE.get_image()).convert('RGB'))
                sample_items[1][mask_len_head + i] = image_trans
...
class Build_Lmdb_Eval_Dataset(Dataset):
    def __init__(self, args, data, item_id_to_keys, db_path, resize):
        self.args = args
        self.data = data
        self.item_id_to_keys = item_id_to_keys
        self.db_path = db_path
    ...
    def __getitem__(self, index):
        ...
        img_processed = img
        return torch.FloatTensor(img_processed)
