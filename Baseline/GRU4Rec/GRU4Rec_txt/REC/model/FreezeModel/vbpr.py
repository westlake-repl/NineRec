...
class VBPR(BaseModel):
    ...
    def forward(self, inputs):
        user, item = inputs
        embed_id_user = self.user_id_embedding(user).unsqueeze(1)   #[5,1,32]
        embed_id_item = self.item_id_embedding(item)   #[5,2,32]

        embed_modal_user = self.user_modal_embedding(user).unsqueeze(1)
        embed_modal_item = self.feature_projection(self.v_feat[item])   #[5,2,32]
