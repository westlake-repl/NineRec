...
class WideDeep(BaseModel):
    ...
    def predict(self, interaction,item_token):
        item_seq, item_len = interaction     #[batch, n_items, seq_len]    [batch]
        batch_size = item_seq.shape[0]   
        item_seq = item_seq.flatten(0,1)    #[batch*n_items, seq_len]
        # item_token = item_token.repeat(batch_size)
        # item_len = item_len.repeat_interleave(self.n_items)
        # item_seq = item_seq.repeat_interleave(self.n_items,0)   #[batch*n_items, seq_len]
        
        # item_seq[:, item_len] = item_token
        ...
        deep_output = self.mlp_layers(deep_input_emb)
        deep_output = self.deep_predict_layer(deep_output)  #[batch*n_items, 1]
