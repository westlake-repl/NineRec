...
class ACF(BaseModel):
    ...
    def __init__(self, config, dataload):
        ...
        v_feat = np.load(self.v_feat_path, allow_pickle=True)   #[item_num, 7,7, 2048]                  
        self.v_feat = torch.tensor(v_feat,dtype=torch.float16).to(self.device)
        self.feature_dim = self.v_feat.shape[-1]
        
        self.item_model = nn.Embedding(self.item_num, self.embedding_size, padding_idx=0,dtype=torch.float16)
    ...
    def forward(self, inputs):
        ...
        logger.info(items)
        logger.info(user_embs.shape)
        logger.info(item_embs.shape)
    ...
    def predict(self, inputs, item_feature):
        ...
        scores = torch.matmul(user_embs, item_feature.t())  # [B n_items]
        return scores
...
class ACFUserNet(nn.Module):
    ...
    def __init__(self, num_users, num_items, emb_dim=128, input_feature_dim=0, profile_embedding=None, device=None):
        ...
        self.user_embedding = nn.Embedding(num_users, emb_dim,dtype=torch.float16)
        ...
        self.w_u = nn.Linear(emb_dim, emb_dim,dtype=torch.float16)
        self.w_v = nn.Linear(emb_dim, emb_dim,dtype=torch.float16)
        self.w_p = nn.Linear(emb_dim, emb_dim,dtype=torch.float16)
        self.w_x = nn.Linear(emb_dim, emb_dim,dtype=torch.float16)
        self.w = nn.Linear(emb_dim, 1,dtype=torch.float16)
    ...
    def forward(self, user_ids, profile_ids, features, profile_mask, return_component_attentions=False,
        ...
        alpha = F.relu(user + profile_query + components) # TODO: + item, Add curent_item emb (?)  #[P, B, D]
        alpha = self.w(alpha)  #[P, B, 1]
...
class ACFFeatureNet(nn.Module):
    ...
    def __init__(self, emb_dim, input_feature_dim, feature_dim, hidden_dim=None, output_dim=None):
        ...
        # e.g. 2048 => 128
        self.dim_reductor = nn.Linear(input_feature_dim, feature_dim,dtype=torch.float16)

        self.w_x = nn.Linear(feature_dim, hidden_dim,dtype=torch.float16)
        self.w_u = nn.Linear(emb_dim, hidden_dim,dtype=torch.float16)

        self.w = nn.Linear(hidden_dim, 1,dtype=torch.float16)
