...
def train(args, use_modal, local_rank):
    ...
    if use_modal:
        if 'roberta' in args.NLP_model_load:
            ...
            nlp_model = RobertaModel.from_pretrained(bert_model_load, config=config)
            if 'base' in args.NLP_model_load:
                pooler_para = [197, 198]
                args.word_embedding_dim = 768
            if 'large' in args.NLP_model_load:
                pooler_para = [389, 390]
                args.word_embedding_dim = 1024
        elif 'opt' in args.NLP_model_load:
            Log_file.info('load opt model...')
            bert_model_load = 'https://huggingface.co/facebook/opt-125M'
            ...
            nlp_model = OPTModel.from_pretrained(bert_model_load, config=config)
            pooler_para = []
        else:
    ...
    if args.is_pretrain == 0:  # pre-trai
