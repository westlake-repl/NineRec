...
class DVBPR(BaseModel):
    ...
    def forward(self, inputs):
        ...
        embed_id_user = self.gamma_users(user).unsqueeze(1)   #[5,1,32]
        embed_id_item = self.gamma_items(item_id)   #[5,2,32]
    ...
    @torch.no_grad()
    def predict(self, user,item_feature):  #[eval_batch]   [num_item, 64]      
        embed_id_user = self.gamma_users(user)  #[5,1,32]
        embed_id_item = self.gamma_items.weight   #[5,2,32]
