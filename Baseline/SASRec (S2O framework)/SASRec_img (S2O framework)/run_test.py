...
def test(args, use_modal, local_rank):
    if use_modal:
        ...
        if 'resnet50' in args.CV_model_load:
            ...
            if 'swin_base' in args.CV_model_load:
    ...
    if True:  # test
        ...
        if 'nextitnet' in args.model_tower:
            # load checkpoint, partial fine tune
            Log_file.info(f'load {args.model_tower} ckpt for test...')
            ...
            Log_file.info('load best_weight...')
            args.is_mp = False
            best_weight_state = best_weight['model_state_dict']
            ...
            best_weight_state = {k: v for k, v in best_weight_state.items() if k in model_state}
            model_state.update(best_weight_state)
            model.load_state_dict(model_state)
            Log_file.info(f"Model loaded from {ckpt_path}")

            # set paras
            torch.set_rng_state(best_weight['rng_state'])
            torch.cuda.set_rng_state(best_weight['cuda_rng_state'])

        elif 'sasrec' in args.model_tower:
            # load checkpoint, partial fine tune
            Log_file.info('load sasrec ckpt for transfer...')
            ckpt_path = get_checkpoint(args.model_path, args.load_ckpt_name)
...
if __name__ == "__main__":
    ...
    # model_dir = args.model_dir  # 只读，不用写checkpoint，所以手动在这这里指定就行
