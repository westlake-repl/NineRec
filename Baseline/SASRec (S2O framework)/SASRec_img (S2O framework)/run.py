...
def train(args, use_modal, local_rank):
    ...
    if use_modal:
        ...
        if 'resnet50' in args.CV_model_load:
            ...
            if 'swin_small' in args.CV_model_load:
                ...
                cv_model_load = '../../pretrained_models/SwinB/'
                cv_model = SwinForImageClassification.from_pretrained(cv_model_load)
            elif 'swin_large' in args.CV_model_load:
                Log_file.info('load swin_large model...')
                cv_model = SwinForImageClassification.from_pretrained('microsoft/swin-large-patch4-window7-224')
    ...
    if args.is_pretrain == 0:  # pre-train任务
    ...
    elif args.is_pretrain == 1:  # transfer任务，读取预训练model参数
    ...
    if use_modal:
        ...
        for index, (name, param) in enumerate(model.module.named_parameters()):
            if param.requires_grad:
                ...
                # if 'user_encoder' in name or 'image_encoder.fc' in name or 'text_encoders.title.fc' in name:
                #     recsys_params.append(param)
                # elif 497 <= index <= 500:  # add_nor, dense
                #     recsys_params.append(param)
                else:
...
if __name__ == "__main__":
    ...
    if 'sasrec' in args.model_tower:
        ...
        log_paras = f'bs_{args.batch_size}_gpu_{args.gpu_device}_ed_{args.embedding_dim}_bn_{args.block_num}'\
                    f'_lr_{args.lr}_Flr_{args.fine_tune_lr}_dp_{args.drop_rate}_L2r_{args.l2_weight_R}_L2m_{args.l2_weight_M}'
