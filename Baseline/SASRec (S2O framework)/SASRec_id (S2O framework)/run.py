...
def train(args, use_modal, local_rank):
    ...
    if use_modal:
        ...
        before_item_id_to_dic, before_item_name_to_id = read_text(
            os.path.join(args.root_data_dir, args.dataset, args.texts), args.which_language)  # read_text
    ...
    if args.is_pretrain == 0:  # pre-train
        ...
        Log_file.info(f'model path {args.model_path}...')
        Log_file.info(f'model path {args.load_ckpt_name}...')
